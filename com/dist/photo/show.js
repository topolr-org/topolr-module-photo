...
Module({
    ...
    event_showloading:function(e){
        this.addChild({
            type:"@loading.area",
            container:this.dom
        }).then(function (area) {
            if(area.dom) {
                area.showLoading(e.data);
            }
        });
    },
    event_closeloading:function () {
        this.getChildByType("@loading.area").remove();
    }
});
Module({
    name:"imageviewer",
    ...
    setImage: function (url) {
        var ths = this;
        this.dispatchEvent("showloading","loading...");
        $.loader().image(url, function () {
            ths.dispatchEvent("closeloading");
